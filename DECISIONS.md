...
## 2026-08-20 — El panel se protege en el cliente, no en el middleware

- **Decisión**: eliminar `apps/web/src/middleware.ts`, que redirigía a `/login` cuando la petición a `/dashboard` no traía la cookie `bingo_session`.
- **Contexto**: al publicar la web en `gramola.daespasa.com` y la API en `gramola-api.daespasa.com`, iniciar sesión dejó de funcionar. El login devolvía 200 y creaba sesión en la base de datos —cuatro seguidas, una por intento—, pero el navegador volvía siempre al acceso.
- **Causa**: la cookie la emite la API sin atributo `Domain`, así que es *host-only* de `gramola-api.daespasa.com`. El servidor de Next, que vive en otro nombre, no la recibe nunca: el middleware evaluaba `cookies.has('bingo_session')` a `false` en todas las peticiones y rebotaba en bucle. En desarrollo no se ve porque web y API comparten `localhost` y **las cookies ignoran el puerto**, de modo que ahí la cookie sí llega a los dos.
- **Alternativa descartada**: emitir la cookie con `Domain=.daespasa.com` para que la vean ambos nombres. Se descarta porque la mandaría también a todo lo demás alojado en ese dominio (Nextcloud, Jellyfin, Plex…), que no tienen nada que ver con Gramola.
- **Consecuencias**: `/dashboard` se protege donde siempre estuvo la autoridad real, que es la API. El cliente ya lo hacía: ante un 401 confirma con `/auth/me` antes de expulsar a nadie, para no echar a quien acaba de entrar por una carrera con la cookie. Se pierde el ahorro de no renderizar el panel a un invitado, a cambio de que la aplicación funcione con web y API en dominios distintos.
